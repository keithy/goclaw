...
if command -v mise &>/dev/null; then
  echo "Mise detected — using mise.podman.toml for env config."
  echo "This file sets env_file = \".env\" to auto-load .env on cd."
  echo ""
  echo "If COMPOSE_FILE is in .env, ensure mise env is active:"
  echo -e "  ${YELLOW}cd /code/goclaw && mise env${NC}"
  echo ""
else
...
if command -v mise; then
  echo -e "${BLUE}─── Mise Configuration ─────────${NC}"
  echo ""
  echo "For mise users, this copies configuration into goclaw:"
  echo "  • options/podman/config/miserc.toml -> .miserc.toml (activates podman environment via mise)"
  echo "  • options/podman/config/mise.podman.toml -> mise.podman.toml (podman settings)"

  if ask "  Copy mise config into goclaw?" y; then
    if [[ ! -f "$GOCLAW_DIR/.miserc.toml" ]]; then
      cp "$SOURCE_DIR/miserc.toml" "$GOCLAW_DIR/.miserc.toml"
      echo -e "  ${GREEN}✓ Copied: .miserc.toml${NC}"
    else
      echo -e "  ${YELLOW}— Skipped (exists): .miserc.toml${NC}"
    fi

    if [[ ! -f "$GOCLAW_DIR/mise.podman.toml" ]]; then
      cp "$SOURCE_DIR/mise.podman.toml" "$GOCLAW_DIR/mise.podman.toml"
      echo -e "  ${GREEN}✓ Copied: mise.podman.toml${NC}"
    else
      echo -e "  ${YELLOW}— Skipped (exists): mise.podman.toml${NC}"
    fi
  fi
fi
